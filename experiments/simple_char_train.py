...
def eval_iteration(model, criterion, val_loader, device):
    model.eval()
    val_loss_total = 0.0
    val_steps = 0
    with torch.no_grad():
...
def main():
    ...
    total_steps = len(train_loader)  # Single epoch
    ...
    for batch in train_loader:
        ...
        if global_step % args.val_interval == 0:
            ...
            avg_val_loss = eval_iteration(model, criterion, val_loader, args.device)
            ...
            print(f"Step {global_step}: Train Loss {avg_train_loss:.4f}, Train PPL {train_perplexity:.4f}, "
                  f"Val Loss {avg_val_loss:.4f}, Val PPL {val_perplexity:.4f}, LR {current_lr:.6f}")
